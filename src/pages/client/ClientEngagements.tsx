...
import { ClientPbcDocumentsModal } from "./ClientPbcDocumentsModal";
...
export const ClientEngagements = () => {
  ...
  const [isClientPbcDocumentsModalOpen, setIsClientPbcDocumentsModalOpen] =
    useState(false);
  ...
  const handleOpenPBC = (engagement) => {
    ...
    setIsClientPbcDocumentsModalOpen(true);
  };

  const handleClosePBC = () => {
    setIsClientPbcDocumentsModalOpen(false);
  };
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      try {
        ...
        const clientFilteredEngagements = allEngagements.filter(
          (eng) => eng.clientId === user?.id
        );
        ...
        for (const engagement of clientFilteredEngagements) {
          try {
            const requests = await documentRequestApi.getByEngagement(
              engagement._id
            );
  ...
  return (
    <div className="min-h-screen bg-amber-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {filteredEngagements.map((engagement) => {
            ...
            return (
              <div
                key={engagement._id}
                className="bg-white/80 border border-white/50 rounded-2xl p-6 hover:bg-white/70 shadow-lg shadow-gray-300/30"
              >
                ...
                <div className="space-y-4">
                  ...
                  {pendingRequests > 0 && (
                    <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-xl border border-gray-200">
                      ...
                      <span className="text-sm text-gray-700 font-medium">
                        {pendingRequests} document request
        ...
        {filteredEngagements.length === 0 && (
          <div className="bg-white/60 backdrop-blur-md border border-white/30 rounded-2xl p-12 text-center shadow-lg shadow-gray-300/30">
            ...
            <p className="text-gray-600">
              {activeTab === "active"
                ? "You have no active audit engagements at the moment."
                : activeTab === "completed"
                ? "You have no completed audit engagements yet."
        ...
        {/* PBC DOCUMENT MODAL */}
        <ClientPbcDocumentsModal
          open={isClientPbcDocumentsModalOpen}
          onOpenChange={setIsClientPbcDocumentsModalOpen}
