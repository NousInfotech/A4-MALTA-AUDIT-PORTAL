...
import { createEmployee, Role } from "@/lib/api/user";

interface User {
  id: string;
  name: string;
  email: string;
  role: "admin" | "employee" | "client" | "partner" | "senior-employee";
...
export const UserManagement = () => {
  ...
  // const roleFilter = searchParams.get('role') || 'all'; // Removed role filter as we have viewType now
  ...
  const filteredUsers = users.filter((user) => {
    ...
    // View Type Filter
    let matchesViewType = false;
    if (viewType === 'clients') {
      matchesViewType = user.role === 'client';
    } else {
      matchesViewType = ['employee', 'partner', 'senior-employee'].includes(user.role);
    }
          ...
          <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Add a new employee, partner, or senior employee to the system.
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCreateEmployee} className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value: Role) => setNewEmployee({ ...newEmployee, role: value })}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value={Role.SENIOR_EMPLOYEE}>Senior Employee</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <DialogFooter>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex p-1 bg-gray-100 rounded-xl w-fit">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${viewType === "clients"
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${viewType === "employees"
                    ? "bg-white text-gray-900 shadow-sm"
                    : "text-gray-500 hover:text-gray-700"
                    }`}
                >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">
                      {viewType === 'clients' ? 'Clients' : 'Employees'}
                    </h3>
                    <p className="text-sm text-gray-600">
                      Manage {viewType} accounts
                    </p>
                  </div>
                </div>
              ...
              <Table className="min-w-[720px]">
                <TableHeader>
                  <TableRow className="border-gray-200">
                    ...
                    {/* Role column removed */}
                    {viewType === "clients" && <TableHead className="text-gray-700 font-semibold">Company</TableHead>}
                  ...
                  {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((user) => (
                      <TableRow key={user.id} className="border-gray-200 hover:bg-gray-50 transition-colors duration-200">
                        ...
                        {/* Role cell removed */}
                        {viewType === "clients" && <TableCell className="max-w-[240px] truncate text-gray-600">
                          {user.companyName || "-"}
                        </TableCell>}
