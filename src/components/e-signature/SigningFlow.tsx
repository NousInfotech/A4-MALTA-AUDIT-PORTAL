...
// 1. Use React.lazy for the dynamic import
const SignatureModal = React.lazy(() =>
  import("@/components/e-signature/SignatureModal").then(
    (mod) => ({ default: mod.SignatureModal }) // Wrap named export in a default object
  )
);
    ...
    return (
      <>
        ...
        <div className="relative w-full bg-slate-50 dark:bg-slate-900">
          ...
          <div
            className={`flex min-h-[85vh] w-full flex-col items-center justify-center gap-8 p-4 text-slate-800 transition-all duration-500 dark:text-slate-200 ${
              !isStarted ? "scale-105 blur-md" : "blur-0 scale-100"
            }`}
          >
            {" "}
            <div className="flex flex-col">
              <Button variant="default" onClick={() => setFlowState("uploading")}>
            ...
            <div className="w-full max-w-lg">
              {!documentToSign ? (
                <div
                  ...
                  onDrop={handleDrop}
                  className={`relative flex w-full flex-col items-center justify-center rounded-xl border-2 border-dashed bg-white p-12 text-center transition-colors duration-300 dark:bg-slate-800 ${
                    isDragging
