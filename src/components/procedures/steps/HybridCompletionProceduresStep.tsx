// File: steps/HybridCompletionProceduresStep.tsx
...
export const HybridCompletionProceduresStep: React.FC<HybridCompletionProceduresStepProps> = ({
  ...
  useEffect(() => {
    ...
    const next = ids.map((sectionId: string) => {
      const base = getPredefinedCompletionSection(sectionId)
      ...
      return {
        id: `hybrid-completion-${sectionId}`,
        sectionId,
        title: base.title,
  ...
  const handleGenerateSectionQuestions = async (sectionId: string) => {
    ...
    try {
      ...
      const res = await authFetch(`${base}/api/completion-procedures/${engagement._id}/generate/hybrid-section-questions`, {
        method: "POST",
        body: JSON.stringify({
          sectionId,
  ...
  const handleGenerateSectionAnswers = async (sectionId: string) => {
    ...
    try {
      ...
      const res = await authFetch(`${base}/api/completion-procedures/${engagement._id}/generate/hybrid-section-answers`, {
        method: "POST",
        body: JSON.stringify({
          sectionId,
          materiality: stepData.materiality || 0,
...
function getPredefinedCompletionSection(sectionId: string) {
