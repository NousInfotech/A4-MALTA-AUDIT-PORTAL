...
import { ConnectionDetailTable } from "./ApideckConnectionDetailTable";
import {
  getJournalEntries,
...
interface ApiCallConfig {
  apiCall: (connectionId: string, consumerId: string) => Promise<any>;
...
export function ApideckDataDisplay({ connectionId }: ApideckDataDisplayProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const fetchApiData = async (config: ApiCallConfig) => {
        ...
        try {
          ...
          const rawData = await config.apiCall(connectionId, user.id);
    ...
    if(user && !authLoading && connectionId) {
        fetchData();
    }
  }, [connectionId, user, authLoading]); // Re-run effect when connectionId changes
