...
// import { Badge } from '@/components/ui/badge';
// import { ArrowLeft, Settings, Link, FileSpreadsheet, Upload, List, History } from 'lucide-react';
// import { Workbook, SheetData, Selection, Mapping, NamedRange } from '../../types/audit-workbooks/types';
...
//   const getCellClassName = useCallback((rowIndex: number, colIndex: number) => {
//     let className = "min-w-[100px] cursor-pointer select-none relative ";
...
//       m.sheet === selectedSheet &&
//       rowIndex >= m.start.row && rowIndex <= m.end.row &&
//       colIndex >= m.start.col && colIndex <= m.end.col
//     );
//     if (mapping) {
//       className += `${mapping.color} `;
//     }
...
//           <div>
//             <h1 className="text-lg font-semibold">{workbook.name}</h1>
//             <p className="text-xs text-gray-500">Version {workbook.version} • Last modified {workbook.lastModified || workbook.uploadedDate}</p>
//           </div>
//         </div>
//         <div className="flex items-center space-x-2">
//           <Button variant="outline" size="sm" onClick={onViewAuditLog}>
...
//                 Link to Field
//               </Button>
//               <Button size="sm" variant="outline" className="w-full justify-start" onClick={onLinkSheet}>
//                 <FileSpreadsheet className="h-4 w-4 mr-2" />
//                 Link Sheet as Dataset
//               </Button>
//               <Button size="sm" variant="outline" className="w-full justify-start" onClick={onLinkWorkbook}>
...
//           <div className="pt-4 border-t">
//             <h3 className="text-sm font-semibold text-gray-600 mb-2">Named Ranges</h3>
//             <div className="space-y-1">
//               {mockNamedRanges.map((nr) => (
//                 <div 
//                   key={nr.name} 
...
//                 >
//                   <span className="font-medium">{nr.name}</span>
//                   <Badge variant="outline" className="text-xs">{nr.range}</Badge>
//                 </div>
//               ))}
//             </div>
//           </div>
...
//             </h3>
//             <div className="space-y-2">
//               {mappings.filter(m => m.sheet === selectedSheet).map((map) => (
//                 <div key={map.id} className={`p-2 text-xs rounded border-l-4 ${map.color}`}>
//                   <p className="font-medium">{map.destinationField}</p>
//                   <p className="text-gray-600">{`${map.sheet}!${currentSheetData[map.start.row]?.[map.start.col]}:${currentSheetData[map.end.row]?.[map.end.col]}`}</p>
//                   {map.transform && <p className="text-gray-500">Transform: {map.transform}</p>}
//                 </div>
//               ))}
//             </div>
//           </div>
...
//           {selection && (
//             <div className="mt-4 p-2 bg-blue-100 text-blue-800 rounded text-sm flex justify-between items-center">
//               <span>Selection: <Badge variant="secondary">{getSelectionText()}</Badge></span>
//               <Button size="sm" onClick={() => onLinkField(selection)}>Link to Field</Button>
//             </div>
//           )}
//         </main>
//       </div>
