...
const ReviewModal: React.FC<ReviewModalProps> = ({
  ...
  const validateForm = () => {
    ...
    if (action === "reopen" && !comments.trim()) {
      // comments is the 'reason' for reopen
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedWorkflow: ReviewWorkflow = await updateReviewWorkflow(
        workflowIdToUse,
        action,
        payload
      );
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {item
              ...
              : "Perform an action on the audit item."}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="grid gap-4 py-4">
          {action === "assign" && (
            <div className="grid gap-2">
              ...
              {errors.reviewerId && (
                <p className="text-red-500 text-sm">{errors.reviewerId}</p>
              )}
            </div>
          )}
          ...
          {action === "review" && (
            <div className="grid gap-2">
                ...
                <Button
                  ...
                  className={`w-full ${
                    approved === true
                      ...
                      : "bg-gray-200 hover:bg-green-300 text-gray-800"
                  }`}
                ...
                <Button
                  ...
                  className={`w-full ${
                    approved === false
                      ...
                      : "bg-gray-200 hover:bg-red-300 text-gray-800"
                  }`}
              ...
              {errors.approved && (
                <p className="text-red-500 text-sm">{errors.approved}</p>
              )}
            </div>
          )}
