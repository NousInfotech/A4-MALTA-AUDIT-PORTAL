...
// // --- Type Definitions (No Changes Here, using existing ones) ---
// type PopulatedAuditItemContent =
//   | {
//       _id: string;
...
//       dueDate?: Date;
//     }
//   | { _id: string; __typename: "KYC"; clientName: string; status: string }
//   | {
//       _id: string;
//       __typename: "ISQMSupportingDocument";
//       title: string;
//       docType: string;
//     }
//   | { _id: string; __typename: "WorkingPaper"; name: string; content: string }
//   | { _id: string; __typename: string; [key: string]: any };

// interface ReviewWorkflowWithPopulatedItem
//   extends Omit<
//     ReviewWorkflow,
//     "itemId" | "engagement" | "itemType" | "status" | "assignedReviewer"
//   > {
//   item: PopulatedAuditItemContent;
//   itemId: string;
...
// // --- Utility Functions for Data Transformation (No Changes Here) ---
// const getItemTitleAndDescription = (
//   item: PopulatedAuditItemContent
// ): { title: string; description: string } => {
//   switch (item.__typename) {
//     case "Procedure":
//     case "PlanningProcedure":
//     case "PBC":
//       return { title: item.name, description: item.description || "" };
//     case "DocumentRequest":
//       return { title: item.requestTitle, description: item.details };
//     case "ChecklistItem":
//     case "ISQMSupportingDocument":
//       return { title: item.title, description: "" };
//     case "KYC":
//       return {
//         title: `KYC for ${item.clientName}`,
...
//       return {
//         title:
//           (item as any).name || (item as any).title || `Item ID: ${item._id}`,
//         description: "",
//       };
//   }
// };
...
// const fetchReviewWorkflowsApi = async (
//   engagementId?: string
// ): Promise<ReviewWorkflowWithPopulatedItem[]> => {
//   try {
//     let response: any;
//     if (engagementId) {
...
//         response.workflows.map(async (wf: any) => {
//           let populatedItem = wf.item;
//           // If item is not populated (or is just an ID/stub), try to fetch details
//           if (
//             !populatedItem ||
//             !populatedItem.__typename ||
//             populatedItem._id === wf.itemId
//           ) {
//             try {
//               const itemDetails = await fetchAuditItemDetails(
//                 wf.itemType,
//                 wf.itemId
//               );
...
//                 populatedItem = {
//                   ...itemDetails.data,
//                   __typename: wf.itemType,
//                   _id: wf.itemId,
//                 };
//               } else {
//                 console.warn(
//                   `Could not fetch details for item ${wf.itemId} of type ${wf.itemType}. Using placeholder.`
//                 );
//                 populatedItem = {
//                   _id: wf.itemId,
//                   __typename: wf.itemType,
//                   name: `Missing ${wf.itemType} details`,
//                 };
//               }
//             } catch (itemErr) {
//               console.error(
//                 `Error fetching details for item ${wf.itemId} of type ${wf.itemType}:`,
//                 itemErr
//               );
//               populatedItem = {
//                 _id: wf.itemId,
...
//           return {
//             ...wf,
//             status: wf.status as ReviewStatus,
//             item: populatedItem,
//             itemId: wf.itemId, // Ensure itemId is always present from the original wf
//             itemType: wf.itemType, // Ensure itemType is always present from the original wf
//           };
//         })
//       );
//       return populatedWorkflows;
//     } else {
...
// const SOCKET_SERVER_URL = "http://localhost:8000"; // Define your Socket.IO server URL
...
//       return `Review Workflows for Engagement: ${urlEngagementId}`;
//     }
...
//         const transformedAuditItems: AuditItem[] = fetchedWorkflows.map(
//           (wf) => {
//             const { title, description } = getItemTitleAndDescription(wf.item);
//             return {
//               _id: wf._id,
//               itemId: wf.itemId,
//               itemType: wf.itemType,
//               engagement: wf.engagement,
//               title: title,
//               description: description,
//               reviewStatus: wf.status,
//               reviewerId: wf.assignedReviewer,
//               reviewerName: wf.reviewerName,
//             };
//           }
//         );
//         setAuditItems(transformedAuditItems);
//         return transformedAuditItems;
//       } catch (err: any) {
...
//         if (revs.success) {
//           setAvailableReviewers(revs.data);
//         } else {
...
//       <h1 className="text-3xl font-bold text-gray-800 mb-6">{displayTitle}</h1>
...
//         refreshAuditItems={() => fetchAndSetReviewWorkflows(selectedFilter)} // Refresh current filtered view
...
//               {" "}
//               {/* Use a grid for vertical stacking */}
...
//               <Select
//                 value={newItemType}
//                 onValueChange={(value: AuditItemType) => setNewItemType(value)}
//               >
...
//                 placeholder="e.g., 65f2a1b3c4d5e6f7a8b9c0d1"
//               />
//             </div>
...
//                 className="w-full" // Make textarea full width
...
// Add a function to fetch all items of a specific type
const fetchItemsByType = async (itemType: AuditItemType) => {
  try {
    let endpoint = "";
    switch (itemType) {
      case "procedure":
      ...
      case "planning-procedure":
        endpoint = "/api/planning-procedures";
        break;
